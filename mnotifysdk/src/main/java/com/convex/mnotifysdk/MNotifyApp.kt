...
object MNotifyApp {
    ...
    private fun initialize(context: Context,intent: Intent, activity: Class<out Activity>, json: String){
        ...
        var configs: MnotifyConfig = readMNotifyConfig(config_json.toString())!!
...
//                    }
    ...
    fun initializeBase(context: Context, activity: Class<out Activity>, json: String) {
        ...
        val configs: MnotifyConfig = readMNotifyConfig(configJson.toString())!!
        ...
        FirebaseMessaging.getInstance().getToken()
            .addOnCompleteListener(object : OnCompleteListener<String?> {
                override fun onComplete(@NonNull task: Task<String?>) {
                    if (!task.isSuccessful) {
                        Log.w("MNotifyApp", "Fetching FCM registration token failed", task.exception)
                        return
                    }
                    ...
                    Log.i("MNotifyApp", "FCM token: $token")
        ...
        isInitialized = true
        Log.i("MNotifyApp", "Base initialization complete")
    }
    ...
    fun handleIntent(context: Context, intent: Intent?) {
        if (intent == null) {
            Log.i("MNotifyApp", "No intent passed to handleIntent()")
            return
        }
    ...
    internal fun readMNotifyConfig(json: String): MnotifyConfig? {
...
//            }
    ...
    fun setOnNotificationClicked(context: Context,intent: Intent){
        Log.d("Notification_SocketIO", "Notification Click: event: MNotifyApp Class")
        if (SocketManager.isConnected()){
            /*            if (intent.extras!=null){
